...
from libs.JAF.plugin_UploadFile import UploadFile, UploadFileParser
...
class UploadFileTest(unittest.TestCase, TestFramework):
    @classmethod
    def setUpClass(cls):
        cls.upload_file = "/tmp/" + "".join(
            random.choices(string.ascii_letters + string.digits, k=20)
        )
        f, cls.local_file = tempfile.mkstemp(text=True)
        os.write(f, b"Test File")
        os.close(f)
    ...
    def setUp(self):
        ...
        self.testcommand = "UploadFile"
        self.TestParserClass = UploadFileParser
        self.TestClass = UploadFile
    ...
    def test_valid_jenkins_anonymous_creds(self):
        ...
        self.basic_test_harness(
            ["jaf.py", self.testcommand, "-s", server, self.local_file, self.upload_file],
    ...
    def test_valid_jenkins_valid_admin_creds(self):
        ...
        self.basic_test_harness(
            ...
            [r"Successfully uploaded file."],
        )


class UploadFileParserTest(unittest.TestCase, TestFramework):
    def setUp(self):
        self.testcommand = "UploadFile"
        self.TestClass = UploadFile
        self.TestParserClass = UploadFileParser
