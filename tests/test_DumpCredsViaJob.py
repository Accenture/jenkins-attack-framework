...
from libs.JAF.plugin_DumpCredsViaJob import DumpCredsViaJob, DumpCredsViaJobParser
...
class DumpCredsViaJobTest(unittest.TestCase, TestFramework):
    @classmethod
    def setUpClass(cls):
        cls.credential_test_job = "credtest" + "".join(
            random.choices(string.ascii_letters + string.digits, k=20)
        )

    def setUp(self):
        warnings.simplefilter("ignore", ResourceWarning)
        self.testcommand = "DumpCredsViaJob"
        self.TestParserClass = DumpCredsViaJobParser
        self.TestClass = DumpCredsViaJob
    ...
    def test_valid_jenkins_invalid_creds(self):
        ...
        self.basic_test_harness(
            ["jaf.py", self.testcommand, "-s", server, "-a", user_bad, self.credential_test_job],
            [r"- \w+: Invalid Credentials or unable to access Jenkins server."],
            1,
        )
    ...
    def test_valid_jenkins_anonymous_creds(self):
        ...
        self.basic_test_harness(
            ["jaf.py", self.testcommand, "-s", server, self.credential_test_job],
            [r"- \w+: Invalid Credentials or unable to access Jenkins server."],
            1,
        )
    ...
    def test_valid_jenkins_valid_admin_creds(self):
        ...
        self.basic_test_harness(
            ["jaf.py", self.testcommand, "-s", server, "-a", user_admin, self.credential_test_job]
        )
    ...
    def test_valid_jenkins_valid_normal_creds(self):
        ...
        self.basic_test_harness(
            ["jaf.py", self.testcommand, "-s", server, "-a", user_normal, self.credential_test_job]
        )
